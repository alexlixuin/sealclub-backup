...
export const Globe = React.memo(function Globe({ globeConfig, data }: WorldProps) {
  ...
  useEffect(() => {
    if (!globeRef.current && groupRef.current) {
      try {
        ...
        console.error("[v0] Globe initialization error:", error)
      }
    }
  }, [])
  ...
  useEffect(() => {
    ...
    intervalRef.current = setInterval(() => {
      ...
      const newNumbersOfRings = genRandomNumbers(0, data.length, Math.min(2, data.length))
...
export function WebGLRendererConfig() {
  ...
  useEffect(() => {
    try {
      ...
      console.error("[v0] WebGL renderer config error:", error)
    }
  }, [gl, size])
...
export const World = React.memo(function World(props: WorldProps) {
  ...
  if (!globeConfig || !props.data) {
    console.error("[v0] World component missing required props")
  ...
  try {
    return (
      <Canvas
        ...
        onError={(error) => console.error("[v0] Canvas error:", error)}
      ...
      >
        ...
        <pointLight color={globeConfig.pointLight} position={new Vector3(-200, 500, 200)} intensity={0.6} />
        <Globe {...props} />
        <OrbitControls
          enablePan={false}
          enableZoom={false}
    ...
    console.error("[v0] World component render error:", error)
    return <div className="flex items-center justify-center h-full text-muted-foreground">Globe unavailable</div>
  }
})
